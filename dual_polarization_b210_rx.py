...
MIMO XPIC: Constant Modulus Algorithm with Cross-Correlation Penalty
  - Input: Raw signal post PFB
  - Output: Recovered I/Q signal
  - Can work with limited reliability at times, if locks onto BPSK, just rerun.

RFML: Binary IQ classifier (1D CNN) trained on real captured data post MIMO XPIC.
  - Input: 1024 complex64 samples -> 2-channel (I,Q) tensor
  - Output: BPSK or QPSK with confidence %

Signal flow:
  USRP ch0 -> AGC -> FLL -> PFB -> MIMO XPIC -> ModClassifier -> Costas(4) -> QPSK decode
  USRP ch1 -> AGC -> FLL -> PFB -> MIMO XPIC -> ModClassifier -> Costas(2) -> BPSK decode
"""
...
# Where model parameters are saved
RFML_MODEL_DIR = r"mimo_model"
# How many samples are used in inference, must match training
RFML_SLICE_LEN = 1024       
# Model predicts every RFML_INTERVAL samples
RFML_INTERVAL = 50000     

# SOI & EOI .jpg parameters
JPEG_START = b"\xFF\xD8"
JPEG_END = b"\xFF\xD9"

# Set the maximum buffer to 2 MB for an image, can increase/
...
class PacketCounter(gr.basic_block):
    ...
    def _handle(self, msg):
        ...
        # Verbosity to print out when a packet is received, useful for debugging.
        if self.count % 50 == 1:
...
class ImageRecoveryBlock(gr.basic_block):
    ...
    # This prevents the image saving too frequently
...
# We use Numba for just in time (jit) compiling for increased efficiency
# This fixed overflow issues on the laptops
...
# Exact model instantiation from training
...
# The rest is derived from compiled GNURadio!
